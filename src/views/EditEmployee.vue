    ...
    methods: {
      ...
      async fetchEmployee() {
        ...
        try {
          ...
          this.employee = response.data[0]; // Asumimos que devuelve un array con un solo empleado
        } catch (error) {
      ...
      async updateEmployee() {
        ...
        formData.append('_method', 'PUT'); // Necesario para Laravel con FormData
        ...
        try {
          ...
          this.success = 'Empleado actualizado con éxito';
          this.error = '';
          setTimeout(() => this.$router.push('/employees'), 2000); // Redirige tras 2 segundos
        } catch (error) {
          this.error = error.response?.data?.error || 'Error al actualizar empleado';
          this.success = '';
        }
      },
      onFileChange(event) {
